...
import { createGroupAction, fetchUserGroups } from "@/app/actions/groupActions";

const GROUP_TYPES: GroupType[] = ["Trip", "Roommates", "Event", "Other"];
const TYPE_EMOJI: Record<GroupType, string> = {
  Trip: "✈️",
  Roommates: "🏠",
  Event: "🎉",
  Other: "💼",
...
function CreateGroupModal({
  ...
  onClose: () => void;
  onCreateGroup: (input: { name: string; type: GroupType; memberNames: string[] }) => Promise<Group>;
}) {
  ...
  const handleMemberChange = (i: number, val: string) =>
    setMembers((p) => p.map((m, idx) => (idx === i ? val : m)));
      ...
      <div className="mx-auto w-full max-w-6xl px-5 py-8 sm:px-8">
        <div className="mb-8">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Manage your shared expenses and settle up
          </p>
        </div>

        {loadingGroups && (
          <div className="mb-6 rounded-2xl border border-white/[0.07] bg-white/[0.035] p-4 text-sm text-slate-400">
            Loading groups...
          </div>
        )}
        ...
        {groups.length > 0 ? (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {groups.map((group: Group) => {
              const gExpenses = expenses.filter(
                (e: Expense) => e.groupId === group.id,
              );
              const total = gExpenses.reduce(
                (s: number, e: Expense) => s + e.amount,
                0,
              );
              return (
                  ...
                  <div className="h-full rounded-2xl border border-white/[0.07] bg-white/[0.035] p-5 transition-all hover:border-indigo-500/40 hover:bg-white/6 hover:-translate-y-1 hover:shadow-[0_8px_32px_rgba(99,102,241,0.15)]">
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        ...
                        <span className="mt-0.5 inline-block rounded-full bg-indigo-500/15 px-2.5 py-0.5 text-xs font-semibold text-indigo-400">
                          {group.type}
                        </span>
                      </div>
                    ...
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-slate-400">
                        {group.members.length} members • {gExpenses.length}{" "}
                        expenses
                      </span>
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="mt-2 max-w-sm text-slate-400">
              Create your first group to start splitting expenses and tracking
              who owes who.
            </p>
